...
const boardShowcase = [
  {
    title: "Main Board 2025-26",
    caption: "Leading innovation,learning, and growth together",
    image: "/25kmainboard.jpeg",
  },
  {
    title: "Main Board 2024-25",
    caption: "Setting the standard for the future boards",
    image: "/ogpeople.jpeg",
  },
  {
    title: "Founder Board Members",
    caption: "The visionaries who started the Code Club legacy",
...
const Members = () => {
    ...
    const fetchMembers = async () => {
      try {
        ...
        const normalizedBoard =
          board in boardConfig ? (board as keyof typeof boardConfig) : "SY";
  ...
  const getBoardTitle = () => {
    return (
      boardConfig[selectedBoard as keyof typeof boardConfig]?.title || "MEMBERS"
    );
  };
  ...
  const otherMembers = useMemo(
    () => members.filter((member) => member.role !== "President"),
    [members],
  );
  ...
  return (
    <div className="bg-zinc-950 min-h-screen w-full">
      <div className="flex flex-col items-center gap-4 pb-4 bg-zinc-950 w-full">
        ...
        <div
          className={`fixed top-0 right-0 h-full bg-zinc-950 w-64 z-50 transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"} md:hidden`}
        >
              ...
              >
                <div className="p-4 flex flex-col items-center h-full">
                  ...
                  <CardHeader className="p-0 text-center mb-2 sm:mb-4 w-full">
                    <CardTitle
                      className={`${member.role === "President" ? "text-lg sm:text-xl" : "sm:text-base text-sm"}`}
                    >
                      {member.name}
                    </CardTitle>
                    <p
                      className={`${member.role === "President" ? "text-base" : "sm:text-sm text-xs"} text-zinc-300`}
                    >
                      {member.role}
                    </p>
                    <p className="text-xs text-zinc-400 break-words px-2">
                      {member.domain}
                    </p>
                  </CardHeader>
